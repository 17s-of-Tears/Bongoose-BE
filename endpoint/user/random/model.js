  ...
  async read(res) {
    await this.dao.serialize(async db => {
      ...
      if(this.requestUserID) {
        ...
        users = await db.get('select distinct user.id, user.email, user.name, user.imageUrl, user.description from user where user.id <> ? and user.id not in (select userRelation.publishUserId from userRelation where userRelation.subscribeUserId=?) order by rand() limit 3', [
